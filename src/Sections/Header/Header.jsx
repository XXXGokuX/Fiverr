...
const company_logo= [ci1,ci2,ci3,ci4,ci5]
const banner_images = [b2, b3, b4, b5]
const banner_text = [
...
const tabs = ["Wordpress", "Website Design", "Logo Design", "AI Services"]
      ...
      <div className='header-inner relative md:h-screen'>
        ...
        <div className='banner-text absolute top-1/4 md:top-1/3 left-[32px] md:right-0 right-[32px]'>
          <div className='heading text-4xl md:text-6xl text-white font-semibold leading-none md:leading-[4.5rem]'>
            <h1>Find the right <span className='font-["auto"] italic'>freelance</span><br /> service, right away</h1>
          </div>
          ...
          <div className='tabs md:block hidden'>
            <ul className='font-semibold list-none flex items-center gap-2 text-white'>
              ...
              {tabs.map(tab => (<li className='rounded-full px-3 py-[2px]  border-[1px] border-white hover:text-gray-700 hover:bg-white cursor-pointer'>{tab}</li>))}
            </ul>
          </div>
        </div>
...
export function CompanyGrid()
{
  return(
    <div className='company-icons bg-[#c2c3c7]/10 py-7 lg:px-52 md:px-28 sm:px-8 px-1'>
      <div className='company-icon-inner grid lg:grid-cols-6 grid-cols-5 gap-3 items-center'>
